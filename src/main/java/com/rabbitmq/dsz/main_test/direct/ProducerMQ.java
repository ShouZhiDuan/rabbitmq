...
public class ProducerMQ {
    ...
    public static void directProducer(Connection connection) throws IOException {
        String directQueueName = "direct-queue-name2";
        ...
        String message = String.format("direct-queue-name2 当前时间：%s", new Date().getTime());
        //参数一：交换机名称。
        //参数二：队列名称。
        //参数三：消息的其他属性-路由的headers信息。
        //参数四：消息主体
        channel.basicPublish("", directQueueName, null, message.getBytes("UTF-8"));
    }
